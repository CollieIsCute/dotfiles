...
委派時可使用可用的最強 Opus 模型，並依任務需要調整思考深度。公司已關閉 Claude Code 的 auto mode 和 dangerously skip permissions；這兩個不能用，不要嘗試，也不要浪費時間繞過。不要送出 secrets、憑證、private keys，或不必要的公司內部資料；也不要讓 Claude Code 執行釋出、推送、刪除、修改 production 等有外部副作用的動作。
