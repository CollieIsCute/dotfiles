...
function __asr_stop_managed --argument-names os
    if test $os = Linux
        command systemctl --user stop crispasr.service >/dev/null 2>&1
    else
        command launchctl remove com.collie.crispasr >/dev/null 2>&1
    end
    or true
end
...
function asr-mode --description 'Switch CrispASR model or stop ASR'
    ...
    if test -z "$cache_dir"
        ...
        test $os = Darwin
        and set cache_dir "$HOME/Library/Caches"
    end
    set -l model_dir "$cache_dir/crispasr"
    ...
    __asr_stop_managed $os
    if test $os = Linux
        command systemd-run --user --quiet --collect --service-type=exec --unit=crispasr.service -- $server_cmd
    else
        command launchctl submit -l com.collie.crispasr -- $server_cmd
    end
    or return
