...
```plantuml
@startuml
title Kitty Theme Fallback Flow

start
:chezmoi renders kitty.conf;

if (Operating system?) then (macOS)
  :include themes/catppuccin-macchiato.conf;
  :Kitty uses Catppuccin Macchiato;
else (Linux)
  :include themes/catppuccin-macchiato.conf;
  :globinclude themes/noctalia.conf;

  if (themes/noctalia.conf exists?) then (yes)
    :Noctalia colors override Catppuccin;
  else (no)
    :Kitty stays on Catppuccin fallback;
  endif

  if (Wallpaper changes through Noctalia?) then (yes)
    :Noctalia resolves wallpaper palette;
    :Render themes/noctalia.conf;
    :post_hook sends SIGUSR1 to Kitty;
    :Kitty reloads config;

    if (Rendered theme exists?) then (yes)
      :Use new Noctalia wallpaper palette;
    else (no)
      :Keep Catppuccin fallback;
    endif
  else (no)
