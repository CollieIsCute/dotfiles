...
- OpenCode Claude Code plugin: temporarily loaded from the local fork build at `~/syncfolder/opencode-claude-code-plugin/dist/index.js` to bridge Claude Code `ExitPlanMode` into OpenCode's native `question` UI. Upstream rollback is `@khalilgharbaoui/opencode-claude-code-plugin@0.5.1`; after changing the plugin path, run `chezmoi apply /home/collie/.config/opencode/opencode.json` and restart OpenCode.
